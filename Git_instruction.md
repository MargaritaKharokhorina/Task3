...
Он возникаеtttт при команде `git pull`, если удаленно и локально есть похожая инфа. В этом случае не происходит автоматического слияния, в связи с чем необходимо выполнить `git commit - m "Resolve conflict" выбрав из предложенных (или отредактировать вручную) путь рарешения конфликта. 
